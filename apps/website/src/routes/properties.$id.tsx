...
export const Route = createFileRoute("/properties/$id")({
  ...
  head: ({ params }) => ({
    meta: [{ title: `عقار رقم ${params.id.slice(0, 8)} | نِفال العقارية` }],
  }),
  ...
  return (
    <div className="min-h-screen flex flex-col">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cover && (
              <button
                ...
                className="block w-full aspect-[16/10] rounded-2xl overflow-hidden bg-muted"
              >
                <img
                  src={cover}
                  alt={data.title}
            ...
            <div className="card-elegant p-6 mt-6 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
                {data.area_sqm && (
                  <Spec icon={Maximize2} label="المساحة" value={`${data.area_sqm} م²`} />
                )}
                {data.bedrooms != null && (
                  <Spec icon={BedDouble} label="غرف النوم" value={data.bedrooms} />
                )}
                {data.bathrooms != null && (
                  <Spec icon={Bath} label="الحمامات" value={data.bathrooms} />
                )}
            ...
            {data.rega_ad_code && (
              <p className="text-xs text-muted-foreground">
                رمز الإعلان (الهيئة العامة للعقار): {data.rega_ad_code}
              </p>
            )}
          </div>
            ...
            <div className="card-elegant p-6 lg:sticky lg:top-20 space-y-4">
              ...
              <a href={`tel:+${COMPANY_WHATSAPP}`} className="block">
                <Button variant="outline" className="w-full h-12">
                  <Phone className="ms-2 h-4 w-4" /> اتصل بنا
                </Button>
              </a>
      ...
      {lightbox && (
        ...
        >
          <button className="absolute top-4 right-4 text-white" aria-label="إغلاق">
            <X className="h-6 w-6" />
          </button>
